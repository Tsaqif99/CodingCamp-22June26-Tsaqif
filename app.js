 ...
 * app.js — all application logic (no build step required)
 *
 * Data is in-memory only: refreshing the page resets everything to zero.
 * Depends on: Chart.js loaded via CDN before this script executes.
 */
...
// ============================================================
// In-Memory State (resets on page refresh — no persistence)
// ============================================================
...
/**
 * Aggregates expense amounts grouped by category.
 * @param {Transaction[]} txArray
 * @returns {CategoryMap}
 */
...
/**
 * Validates form input data before creating a transaction.
...
/**
 * Formats a transaction's amount as a signed string with two decimal places.
...
/**
 * Appends a new transaction to the in-memory array and returns the updated array.
...
/**
 * Removes the transaction with the given id and returns the updated array.
 * @param {string} id
 * @returns {Transaction[]}
 */
...
/**
 * Re-renders balance, transaction list, and chart from the current transactions array.
 */
function render() {
  ...
  if (listEl) {
    // Remove all existing transaction items (keep the empty-state li)
...
/**
 * Creates and stores the Chart.js pie instance.
 * Guards against Chart.js CDN failure.
 */
function initChart() {
  ...
  // Hide canvas initially — render() will show it when there's data
...
/**
 * Handles form submission: validates, creates transaction, re-renders, resets form.
 * @param {Event} event
 */
...
function capitalize(str) {
  ...
  return labelMap[str] || str.charAt(0).toUpperCase() + str.slice(1);
}
